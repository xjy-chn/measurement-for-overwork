...
twoway (tsline MEAN, lcolor(%80) lwidth(vthin) tlabel(#19,angle(forty_five))  ttick(08Jan2012 26Jan2013 16Jan2014 04Feb2015 24Jan2016 13Jan2017 01Feb2018 21Jan2019 10Jan2020 16Feb2012 06Mar2013 24Feb2014 15Mar2015 03Mar2016 21Feb2017 12Mar2018 01Mar2019 18Feb2020, tpos(in) )) ///
 ...
 xsize(80) ysize(30) legend(label(1 超时加班情况) label(2 年度最高点) label(3 年度最低点) )  title("daily overwork") xtitle("date") ytitle("overwork")
...
*年度极值点
// addplot: scatter  MEAN date2 if tops==1,  ms(o) mcolor(red) mlabangle(45) mlabsize(small)  mlabel(labmean2) legend("年度最大值")      //追加最近一个交易日涨跌幅
// addplot: scatter  MEAN date2 if bottoms==1,  ms(d) mlabangle(45) mcolor(black) mlabel(labmean2)  legend("年度最小值")    //追加最近 120 日交易日最高价时的涨跌幅


**2013
// 
// addplot: scatter  MEAN date2 if x3<1,  ms(o)   mlabel(MEAN) legend(off)      //追加最近一个交易日涨跌幅
// addplot: scatter  MEAN date2 if x5<1,  ms(d)   mlabel(MEAN)  legend(off)    //追加最近 120 日交易日最高价时的涨跌幅
// addplot: scatter  MEAN date2 if x7<1,  ms(d)   mlabel(MEAN)  legend(off)    //追加最近 120 日交易日最低价时的涨跌幅


addplot : pcarrowi  0.75 19155 0.55 19355 "Revision of the Labor Contract Law" ///
0.6 21347 0.4 21547  "Revision of the Labor Law" ///
0.8 21600 0.58 21921 "Initial extraction of COVID-19" ///
 ,legend(label("超时加班情况") ) 
 
graph save 全国全年加班情况-春运.gph,replace
 
 **单独绘制事件
 twoway (tsline MEAN if date2>=19340&date2<=19360, lcolor(%80) lwidth(vthin))  , ///
 xsize(30) ysize(30) title("daily overwork") xtitle("date") ytitle("overwork") tline(19355, lp(dash) lc(red))
