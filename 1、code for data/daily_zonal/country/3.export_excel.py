...
if __name__=="__main__":
    ...
    for year in range(2012,2021):
        if not os.path.exists(rf"F:\日度夜间灯光\原始数据\result\日度分区统计\deep\{radius}x{radius}\全国企业数量加权\{year}"):
            os.makedirs(rf"F:\日度夜间灯光\原始数据\result\日度分区统计\deep\{radius}x{radius}\全国企业数量加权\{year}")
    #拼合日度加班情况
        ...
        for day in days:
            ...
            arcpy.conversion.TableToExcel(rf"F:\popLight\日度灯光分区统计\daily_zontal.gdb\deep_swfn{year}{day}",
                                          rf"F:\日度夜间灯光\原始数据\result\日度分区统计\deep\{radius}x{radius}\全国企业数量加权\{year}\{day}.xlsx", "NAME", "CODE")
