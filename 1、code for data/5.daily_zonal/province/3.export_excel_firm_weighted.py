...
if __name__=="__main__":
    ...
    for year in range(2012,2021):
        if not os.path.exists(rf"./zonal/{radius}x{radius}/prov_weighted/{year}"):
            os.makedirs(rf"./zonal/{radius}x{radius}/prov_weighted/{year}")
        days = os.listdir(fr'F:\ntl\raw\{year}')
        days=[day for day in days if day[-4:]!='.csv']
        for day in days:
            arcpy.conversion.TableToExcel(rf"F:\overwork\daily_zontal.gdb\daily_zontal.gdb\deep2_swfp{year}{day}",
                                          rf"./zonal/{radius}x{radius}/prov_weighted/{day}.xlsx", "NAME", "CODE")
